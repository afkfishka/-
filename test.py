...
class Player(sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.direction = False  # Направление движения

    def start_move(self, left, right, up, down):
        if left or right:
            self.direction = True
    ...
    def update(self, platforms):
        ...
        if self.direction:
            ...
            if not moved:
                self.direction = False  # Останавливаем движение, если столкновение
...
def main():
    ...
    while True:
        ...
        hero.start_move(keys[K_LEFT], keys[K_RIGHT], keys[K_UP], keys[K_DOWN])
